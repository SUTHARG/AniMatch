...
import 'package:animatch/presentation/widgets/rating_sheet.dart';
import 'package:animatch/core/utils/streaming_utils.dart';
...
class _DetailScreenState extends State<DetailScreen> {
  ...
  Future<void> _launchUrl(String rawUrl,
    ...
    final resolvedRaw = platformName != null
        ? resolveStreamingUrl(platformName, rawUrl)
        : rawUrl;
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _media = media;
          if (widget.isManga) {
            _readStatus = entry != null
                ? ReadStatus.fromString(entry['status'] as String?)
                : null;
  ...
  void _share() {
    ...
    final text =
        'Check out "${media.displayTitle}" on AniMatch!\nType: $type · Score: ${media.scoreText} · $progress';
                  ...
                  child: (_loadingAnilist && _anilistImageUrl == null)
                      ...
                      : Stack(
                          ...
                          children: [
                            PremiumImage(
                              imageUrl:
                                  _anilistImageUrl ?? media.displayImageUrl,
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      if (!widget.isManga)
                        _Badge(
                            label: (media as Anime).rating?.split(' ').first ??
                                'NR',
                      ...
                      if (widget.isManga)
                        _Badge(
                            label: 'vol ${(media as Manga).volumes ?? "?"}',
                            color: const Color(0xFFE5B1D5),
                            textColor: Colors.black),
                      Text('•',
                          style: TextStyle(
                              color: Colors.white.withValues(alpha: 0.5))),
                      Text(media.mediaTypeBadge,
                          style: TextStyle(
                              color: Colors.white.withValues(alpha: 0.5))),
                  ...
                  if (_media is Anime &&
                      (_media as Anime).trailerUrl != null &&
                      (_media as Anime).trailerUrl!.isNotEmpty) ...[
                    const SizedBox(height: 12),
                    SizedBox(
                      width: double.infinity,
                      child: OutlinedButton.icon(
                        onPressed: () =>
                            _launchUrl((_media as Anime).trailerUrl!),
                  ...
                  Align(
                      ...
                      child: Text('Overview:',
                          style: theme.textTheme.titleMedium?.copyWith(
                              ...
                              color: Colors.white))),
                  const SizedBox(height: 12),
                  Text(_media?.synopsis ?? 'No description available.',
                      style: TextStyle(
                  ...
                  _DetailItem(
                      label: 'Japanese:',
                      value: (widget.isManga
                              ? (media as Manga).titleJapanese
                              : (media as Anime).titleJapanese) ??
                          'N/A'),
                  _DetailItem(
                      label: 'Synonyms:',
                      value: media is Anime
                          ? (media).synonyms.join(', ')
                          : (media as Manga).synonyms.join(', ')),
                  _DetailItem(
                      label: widget.isManga ? 'Published:' : 'Aired:',
                      value: (widget.isManga
                              ? (media as Manga).publishedString
                              : (media as Anime).airedString) ??
                          'N/A'),
                  if (!widget.isManga)
                    _DetailItem(
                        label: 'Premiered:',
                        value: (media as Anime).premiered ?? 'N/A'),
                  if (!widget.isManga)
                    _DetailItem(
                        label: 'Duration:',
                        value: (media as Anime).duration ?? 'N/A'),
                  _DetailItem(
                      label: 'Status:',
                      value: media.isCompleted
                          ? 'Finished'
                          : (media.isOngoing
                              ? (widget.isManga
                                  ? 'Publishing'
                  ...
                  if (widget.isManga) ...[
                    _DetailItem(
                        label: 'Authors:',
                        value: (media as Manga).authors.join(', ').isEmpty
                            ? 'N/A'
                            : (media).authors.join(', ')),
                    _DetailItem(
                        label: 'Serialization:',
                        value: (media).serializations.join(', ').isEmpty
                            ? 'N/A'
                            : (media).serializations.join(', ')),
                  ] else ...[
                    _DetailItem(
                        label: 'Studios:',
                        value: (media as Anime).studios.join(', ').isEmpty
                            ? 'N/A'
                            : (media).studios.join(', ')),
                    _DetailItem(
                        label: 'Producers:',
                        value: (media).producers.isEmpty
                            ? 'N/A'
                            : (media).producers.take(3).join(', ')),
                  ],
                  const SizedBox(height: 32),
                  if (inList &&
                      ((!widget.isManga &&
                              _watchStatus == WatchStatus.watching) ||
                          (widget.isManga &&
        ...
        children: [
          ...
          Row(
            children: [
              ...
              IconButton(
                  onPressed:
                      current < total ? () => onChanged(current + 1) : null,
...
class _UserRatingCard extends StatelessWidget {
  ...
  const _UserRatingCard(
      {required this.rating, this.review, required this.onEdit});
  @override
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.all(16),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => Navigator.pushReplacement(
          ...
          MaterialPageRoute(
              builder: (_) =>
                  DetailScreen(malId: media.malId, isManga: media is Manga))),
      child: Container(
        width: 140,
        margin: const EdgeInsets.only(right: 16),
        child: Column(
